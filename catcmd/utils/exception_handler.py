from catcmd.config import _EXCEPTION_LIST_

# Generates all exceptions in projectaile, used for logging.
class EXCEPTION_HANDLER:
    def __init__(self):
        self._EXCEPTION_LIST_ = _EXCEPTION_LIST_
    ...
    def generate_exception(
        ...
        exception_source_exists = exception_source in self._EXCEPTION_LIST_.keys()
        
        exception_source = exception_source if exception_source_exists else 'default'
        
        exception_exists = exception_name in self._EXCEPTION_LIST_[exception_source].keys()
        
        exception_name = exception_name if exception_exists else 'base_exception'
        
        base_exception_string = self._EXCEPTION_LIST_[exception_source][exception_name] 
